...
def parse():
    ...
    p.add_argument(
        "--pseudotime_txt",
        required=True,
        help="Path to PseudoTime.txt for cell ordering",
    )
...
def main():
    ...
    with h5py.File(out, "w") as h5:
        ...
        gX.create_dataset(
            "indices", (0,), maxshape=(None,), dtype="uint32", chunks=True
        )
        ...
        g_obs.create_dataset(
            "_index", (0,), maxshape=(None,), dtype="S32"
        )  # gene names
        ...
        for r, line in enumerate(fh, 1):
            ...
            if len(buf_rows) >= args.buffer:
                ...
                buf_rows, data_collected, idx_collected, indptr, csv_rows = (
                    [],
                    [],
                    [],
                    [nnz],
        ...
        with h5py.File(out, "r") as f_debug:
            ...
            if "obs" in f_debug:
                ...
                if "_index" in f_debug["obs"]:
                    print(
                        f"[DEBUG] 'obs/_index' shape: {f_debug['obs']['_index'].shape}"
                    )
            else:
...
def flush(
    h5, ds_data, ds_idx, ds_indptr, ds_obs, rows, datas, idxs, indptr, final=False
):
    if not rows:
        return
