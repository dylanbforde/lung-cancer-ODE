...
output_pseudotime_csv = (
    "/home/dylan/Code/lung-cancer-ODE/Beeline/inputs/q3_data/LungCancer/PseudoTime.csv"
)
...
if not os.path.exists(input_expr_csv):
    ...
    if not cell_ids:
        ...
        df_pseudotime = pd.DataFrame(
            {"CellID": cell_ids, "PseudoTime": range(len(cell_ids))}
        )
        df_pseudotime.to_csv(
            output_pseudotime_csv, index=False
        )  # Do not write DataFrame index
        print(
            f"Dummy PseudoTime.csv created at {output_pseudotime_csv} with {len(cell_ids)} cells."
        )
